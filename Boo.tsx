...
export function Boo({ channel }: { channel: Channel; }) {
    ...
    useEffect(() => {
        ...
        // Also exclude Clyde messages
        const isClyde = lastMessage.author.username === "Clyde";
        if (isExempted || (settings.store.ignoreBots && lastMessage.author.bot) || isClyde) {
            if (countedChannels.has(id)) {
                countedChannels.delete(id);
                setBooCount(getBooCount() - 1);
            }
    ...
    const isClyde = lastMessage.author.username === "Clyde";
    if (!state.isDataProcessed || !currentUserId || !lastMessage || state.isCurrentUser || isChannelExempted(id) || isCleared || (settings.store.ignoreBots && lastMessage.author.bot) || isClyde)
        return null;
